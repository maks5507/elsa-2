...
class BaseTokenizerWithMapping:
    ...
    def tokenize(self, sentences: List[str]) -> Tuple[torch.Tensor, List[int]]:
        ...
        num_padding_tokens_to_add = 512 - 2 - len(tokenized_sequence)
        tokenized_sequence += [self.pad_token_id] * num_padding_tokens_to_add
        mapping += [-2] * num_padding_tokens_to_add
